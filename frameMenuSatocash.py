...
class FrameMenuSatocash(customtkinter.CTkFrame):
    def __init__(self, master):
        super().__init__(master)

        logger.debug("FrameMenuSatocash init")
        try:
            self.configure(
            ...
            self.button_balance = master.create_menu_button(
                self, "Balances",
                "balance.png",  # todo
                0.33, 0.05,
                state="normal",
                command=lambda: [
                    master.show_satocash_balances(),
                ]
            )

            self.button_import_tokenv4 = master.create_menu_button(
                self, "Import tokenv4",
                "import.png",
                0.40, 0.05,
                state="normal",
                command=lambda: [
                    master.show_satocash_import_tokenv4(),
                ]
            )

            self.button_export_tokenv4 = master.create_menu_button(
                self, "Export tokenv4",
                "export.png",
                0.47, 0.05,
                state="normal",
                command=lambda: [
                    master.show_satocash_export_tokenv4(),
                ]
            )
            ...
            logger.error(f"Unexpected error in FrameMenuSatocash init: {e}", exc_info=True)
