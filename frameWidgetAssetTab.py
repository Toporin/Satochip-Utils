...
class FrameWidgetAssetTab(customtkinter.CTkTabview):
    def __init__(self, master, width, height, **kwargs):
        ...
        try:
            ...
            self.set("Token")
            self.token_table = FrameWidgetAssetTable(master=token_tab, width=table_width, height=table_height)
            self.token_table.place(relx=0.0, rely=0.0, anchor="nw")

            # nft
            self.nft_table = FrameWidgetAssetTable(master=nft_tab, width=table_width, height=table_height)
            self.token_table.place(relx=0.0, rely=0.0, anchor="nw")
