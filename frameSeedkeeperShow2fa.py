...
class FrameSeedkeeperShow2fa(customtkinter.CTkFrame):
    def __init__(self, master):
        super().__init__(master)

        logger.debug("FrameSeedkeeperShow2fa init")
        ...
        try:
            ...
            self.secret2FA_label = master.create_label("2FA:", frame=self)
            self.secret2FA_label.place(relx=0.05, rely=rely, anchor="nw")
            rely += 0.05
            self.secret2FA_textbox = master.create_textbox(frame=self)
            self.secret2FA_textbox.place(relx=0.05, rely=rely, relheight=0.23, anchor="nw")
    ...
    def update(self, secret):
        ...
        secret2FA = ""
        try:
            secret2FA = secret['secret'][2:]
        except Exception as e:
            logger.error(f"Error decoding 2FA: {e}", exc_info=True)
            secret2FA = f"Failed to parse secret: {e}"
        self.secret2FA_textbox.insert("1.0", '*' * len(secret2FA)) # Masque la valeur

        # qr-code
        self.qr_button.configure(
            command=lambda params=(secret2FA, self.qr_label): show_qr_code(params[0], params[1])
        )

        self.show_button.configure(
            command=lambda txt=secret2FA: [
                toggle_textbox_visibility(self.secret2FA_textbox, txt),
            ]
        )
