...
from utils import show_qr_code, toggle_textbox_visibility

logger = logging.getLogger(__name__)
logger.setLevel(logging.DEBUG)


class FrameSeedkeeperShowData(customtkinter.CTkFrame):
    def __init__(self, master):
        super().__init__(master)

        logger.debug("FrameSeedkeeperShowData init")
        ...
        try:
            ...
            self.header = master.create_an_header(
                "Data details",
                "secrets_icon_popup.png", # todo use data icon
                frame=self
            )
            ...
            self.data_label = master.create_label("Data:", frame=self)
            self.data_label.place(relx=0.05, rely=rely, anchor="nw")
            rely += 0.05
            self.data_textbox = master.create_textbox(frame=self)
            self.data_textbox.place(relx=0.05, rely=rely, relheight=0.23, anchor="nw")
    ...
    def update(self, secret):
        ...
        data = ""
        try:
            secret = self.master.controller.decode_free_text(secret)
            data = secret['text']
        except Exception as e:
            logger.error(f"Error decoding data: {e}", exc_info=True)
            mnemonic = f"Failed to parse secret: {e}"
        self.data_textbox.insert("1.0", '*' * len(data)) # Masque la valeur

        # qr-code
        self.qr_button.configure(
            command=lambda params=(data, self.qr_label): show_qr_code(params[0], params[1])
        )

        self.show_button.configure(
            command=lambda txt=data: [
                toggle_textbox_visibility(self.data_textbox, txt),
            ]
        )
